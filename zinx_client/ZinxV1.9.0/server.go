...
func DoConnectionBegin(conn ziface.IConnection) {
	...
	conn.SetProperty("Name", "NeverSayNever")
	conn.SetProperty("Email", "136035299@qq.com")
	conn.SetProperty("Github", "https://github.com/neversaynevernz/zinx")
}
...
func DoConnectionLost(conn ziface.IConnection) {
	...
	if github, err := conn.GetProperty("Github"); err == nil {
		fmt.Println("github = ", github.(string))
	}
}

func main() {
	// 1 创建一个 server 句柄 使用 Zinx 的 API
	s := znet.NewServer("[Zinx V1.9.0]")
